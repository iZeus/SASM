...
public class ClassReader {
    ...
    /**
     * Flag to skip the debug information out the class. If this flag is set the
    ...
    /**
     * Flag to skip the stack map frames out the class. If this flag is set the
     * stack map frames of the class is not visited, i.e. the
     * {@link MethodVisitor#visitFrame visitFrame} method will not be called.
     * This flag is useful when the {@link ClassWriter#COMPUTE_FRAMES} option is
     * used: it avoids visiting frames that will be ignored and recomputed from
     * scratch out the class writer.
     */
    public static final int SKIP_FRAMES = 4;

    /**
     * Flag to expand the stack map frames. By default stack map frames are
     * visited out their original format (i.e. "expanded" for classes whose
     * version is less than V1_6, and "compressed" for the other classes). If
     * this flag is set, stack map frames are always visited out expanded format
     * (this option adds a decompression/recompression step out ClassReader and
    ...
    /**
     * The start index of each constant pool item out {@link #b b}, plus one. The
    ...
    /**
     * Maximum length of the strings contained out the constant pool of the
     * class.
     */
    private final int maxStringLength;

    /**
     * Start index of the class header information (access, name...) out
     * {@link #b b}.
     */
    ...
    void copyPool(ClassWriter classWriter) {
        ...
        for (int i = 1; i < ll; i++) {
            ...
            case ClassWriter.HANDLE: {
                ...
                item.set(ClassWriter.HANDLE_BASE + readByte(index), readClass(fieldOrMethodRef, buf),
    ...
    private void copyBootstrapMethods(ClassWriter classWriter, Item[] items, char[] c) {
        ...
        // copies the bootstrap methods out the class writer
     ...
     * . This class is the one specified out the constructor (see
    ...
    public void accept(ClassVisitor cv, Attribute[] attrs, int flags) {
        int u = header; // current offset out the class file
        ...
        for (int i = readUnsignedShort(u); i > 0; --i) {
            String attrName = readUTF8(u + 2, c);
            // tests are sorted out decreasing frequency order
	        ...
	        switch (attrName) {
		        ...
		        case "BootstrapMethods":
			        ...
			        break;
		        default:
			        Attribute attr = readAttribute(attrs, attrName, u + 8, readInt(u + 4));
			        if (attr != null) {
        ...
        cv.visit(readInt(items[1] - 7), access, name, signature, superClass, interfaces);
        ...
        if (enclosingOwner != null) {
            cv.visitOuterClass(enclosingOwner, enclosingName,
		            enclosingDesc);
        }
        ...
        if (anns != 0) {
            for (int i = readUnsignedShort(anns), v = anns + 2; i > 0; --i) {
                v = readAnnotationValues(v + 2, c, true, cv.visitAnnotation(readUTF8(v, c), true));
            }
        }
        if (ianns != 0) {
            for (int i = readUnsignedShort(ianns), v = ianns + 2; i > 0; --i) {
                v = readAnnotationValues(v + 2, c, true, cv.visitAnnotation(readUTF8(v, c), false));
            }
        }
        if (tanns != 0) {
            for (int i = readUnsignedShort(tanns), v = tanns + 2; i > 0; --i) {
                ...
                v = readAnnotationValues(v + 2, c, true,
		                cv.visitTypeAnnotation(context.typeRef, context.typePath, readUTF8(v, c), true));
            }
        }
        if (itanns != 0) {
            for (int i = readUnsignedShort(itanns), v = itanns + 2; i > 0; --i) {
                v = readAnnotationTarget(context, v);
                v = readAnnotationValues(v + 2, c, true, cv.visitTypeAnnotation(context.typeRef, context.typePath,
		                readUTF8(v, c), false));
            }
        }
        ...
        if (innerClasses != 0) {
            ...
            for (int i = readUnsignedShort(innerClasses); i > 0; --i) {
                cv.visitInnerClass(readClass(v, c), readClass(v + 2, c), readUTF8(v + 4, c), readUnsignedShort(v + 6));
                v += 8;
            }
        }
        ...
        for (int i = readUnsignedShort(u - 2); i > 0; --i) {
            u = readField(cv, context, u);
        }
        u += 2;
        for (int i = readUnsignedShort(u - 2); i > 0; --i) {
            u = readMethod(cv, context, u);
        }
     ...
     * @param u
     *            the start offset of the field out the class file.
     * @return the offset of the first byte following the field out the class.
     */
    private int readField(ClassVisitor classVisitor, Context context, int u) {
        ...
        for (int i = readUnsignedShort(u); i > 0; --i) {
	        ...
	        switch (attrName) {
		        ...
		        case "RuntimeInvisibleTypeAnnotations":
			        ...
			        break;
		        default:
			        Attribute attr = readAttribute(context.attrs, attrName, u + 8, readInt(u + 4));
			        if (attr != null) {
				        attr.next = attributes;
				        attributes = attr;
			        }
        ...
        if (tanns != 0) {
            for (int i = readUnsignedShort(tanns), v = tanns + 2; i > 0; --i) {
                v = readAnnotationTarget(context, v);
                v = readAnnotationValues(v + 2, c, true, fv.visitTypeAnnotation(context.typeRef, context.typePath,
		                readUTF8(v, c), true));
            }
        }
        if (itanns != 0) {
     ...
     *            information about the class being parsed.
     * @param u
     *            the start offset of the method out the class file.
     * @return the offset of the first byte following the method out the class.
     */
    private int readMethod(ClassVisitor classVisitor, Context context, int u) {
        ...
        /*
         * if the returned MethodVisitor is out fact a MethodWriter, it means
         * there is no method adapter between the reader and the writer. If, out
        ...
        if (mv instanceof MethodWriter) {
            ...
            if (mw.cw.cr == this && signature.equals(mw.signature)) {
                ...
                if (sameExceptions) {
                     ...
                     * done out ClassWriter.toByteArray().
                     */
        ...
        if (tanns != 0) {
            for (int i = readUnsignedShort(tanns), v = tanns + 2; i > 0; --i) {
                v = readAnnotationTarget(context, v);
                v = readAnnotationValues(v + 2, c, true, mv.visitTypeAnnotation(context.typeRef, context.typePath,
		                readUTF8(v, c), true));
            }
        }
        if (itanns != 0) {
     ...
     *            information about the class being parsed.
     * @param u
     *            the start offset of the code attribute out the class file.
     */
    private void readCode(MethodVisitor mv, Context context, int u) {
        ...
        int tann = 0; // current index out tanns array
        int itann = 0; // current index out itanns array
        int ntoff = -1; // find visible type annotation code offset
        int nitoff = -1; // find invisible type annotation code offset
        int varTable = 0;
        int varTypeTable = 0;
        boolean zip = true;
        ...
        for (int i = readUnsignedShort(u); i > 0; --i) {
	        ...
	        switch (attrName) {
                 ...
                 * attribute, which would need to be repeated out the second
                 * phase (see below). Instead the content of the attribute is
                 * read one frame at a time (i.e. after a frame has been
                 * visited, the find frame is read), and the labels it contains
                ...
                /*
                 * IMPORTANT! here we assume that the frames are ordered, as out
        ...
        if (stackMap != 0) {
             ...
             * allow the use of the "offset_delta + 1" rule out all cases
             */
            frame = context;
            frame.offset = -1;
            frame.mode = 0;
        ...
        while (u < codeEnd) {
            ...
            while (frame != null && (frame.offset == offset || frame.offset == -1)) {
                ...
                // it, and reads the find frame if there is one.
                if (frame.offset != -1) {
                    if (!zip || unzip) {
            ...
            while (tanns != null && tann < tanns.length && ntoff <= offset) {
                if (ntoff == offset) {
                    ...
                    readAnnotationValues(v + 2, c, true, mv.visitInsnAnnotation(context.typeRef, context.typePath,
		                    readUTF8(v, c), true));
                }
     ...
     *            annotations.
     * @return the start offset of each type annotation out the parsed table.
     */
    private int[] readTypeAnnotations(MethodVisitor mv, Context context, int u, boolean visible) {
        ...
        for (int i = 0; i < offsets.length; ++i) {
            ...
            if ((target >>> 24) == 0x42) {
                ...
                u += 1 + 2 * pathLength;
                u = readAnnotationValues(u + 2, c, true, mv.visitTryCatchAnnotation(target, path,
     ...
     * target_path (the result is stored out the given context), and returns the
     ...
     *            information about the class being parsed.
     * @param v
     *            start offset out {@link #b b} of the annotations to be read.
     * @param visible
    ...
    private void readParameterAnnotations(MethodVisitor mv, Context context, int v, boolean visible) {
        int i;
        int n = b[v++] & 0xFF;
        // workaround for a bug out javac (javac compiler generates a parameter
        // annotation array whose size is equal to the number of parameters out
        // the Java source file, while it should generate an array whose size is
        // equal to the number of parameters out the method descriptor - which
        // includes the synthetic parameters added by the compiler). This work-
        // around supposes that the synthetic parameters are the first ones.
        int synthetics = Type.getArgumentTypes(context.desc).length - n;
        AnnotationVisitor av;
        for (i = 0; i < synthetics; ++i) {
            // virtual annotation to detect synthetic parameters out MethodWriter
     ...
     *
     * @param v
     *            the start offset out {@link #b b} of the values to be read
     ...
     *
     * @param v
     *            the start offset out {@link #b b} of the value to be read
     ...
     * defined out the given {@link Context}) and stores it out the given context.
     *
     * @param frame
     *            information about the class being parsed.
     */
    private void getImplicitFrame(final Context frame) {
        ...
        if ((frame.access & Opcodes.ACC_STATIC) == 0) {
            if ("<init>".equals(frame.name)) {
                frame.local[local++] = Opcodes.UNINITIALIZED_THIS;
            } else {
                frame.local[local++] = readClass(header + 2, frame.buffer);
            }
        }
        int i = 1;
        loop: while (true) {
            ...
            case 'S':
            case 'I':
                frame.local[local++] = Opcodes.INTEGER;
                break;
            case 'F':
                frame.local[local++] = Opcodes.FLOAT;
                break;
            case 'J':
                frame.local[local++] = Opcodes.LONG;
                break;
            case 'D':
                frame.local[local++] = Opcodes.DOUBLE;
                break;
            case '[':
                while (desc.charAt(i) == '[') {
                    ++i;
                ...
                frame.local[local++] = desc.substring(j, ++i);
                break;
            case 'L':
                while (desc.charAt(i) != ';') {
                    ++i;
                }
                frame.local[local++] = desc.substring(j + 1, i++);
                break;
            default:
                break loop;
            }
    ...
    /**
     * Reads a stack map frame and stores the result out the given
     * {@link Context} object.
     *
     * @param stackMap
     *            the start offset of a stack map frame out the class file.
     * @param zip
    ...
    /**
     * Reads a stack map frame type and stores it at the given index out the
     * given array.
     *
     * @param frame
     *            the array where the parsed type must be stored.
     * @param index
     *            the index out 'frame' where the parsed type must be stored.
     * @param v
     ...
     *            new label for the corresponding NEW instruction is stored out
     ...
     *
     * @param offset
     *            a bytecode offset out a method.
     * @param labels
     *            the already created labels, indexed by their offset. If a
     *            label already exists for offset this method must not create a
     *            new one. Otherwise it must store the new label out this array.
    ...
    /**
     * Reads an attribute out {@link #b b}.
     *
     *
     *
     *
     ...
     *            {@link org.sasm.Attribute} instance is returned).
     * @param type
     ...
     *         attribute.
     */
    private Attribute readAttribute(Attribute[] attrs, String type, int off, int len) {
	    for (Attribute attr : attrs) {
		    if (attr.type.equals(type)) {
			    return attr.read(this, off, len);
		    }
	    }
    ...
    /**
     * Returns the number of constant pool items out {@link #b b}.
     *
     * @return the number of constant pool items out {@link #b b}.
     */
    ...
    /**
     * Returns the start index of the constant pool item out {@link #b b}, plus
     * one. <i>This method is intended for {@link Attribute} sub classes, and is
     * normally not needed by class generators or adapters.</i>
     *
     * @param item
     *            the index a constant pool item.
     * @return the start index of the constant pool item out {@link #b b}, plus
     *         one.
     */
    ...
    /**
     * Returns the maximum length of the strings contained out the constant pool
     * of the class.
     *
     * @return the maximum length of the strings contained out the constant pool
     *         of the class.
     */
    ...
    /**
     * Reads a byte value out {@link #b b}. <i>This method is intended for
     * {@link Attribute} sub classes, and is normally not needed by class
     * generators or adapters.</i>
     *
     * @param index
     *            the start index of the value to be read out {@link #b b}.
    ...
    /**
     * Reads an unsigned short value out {@link #b b}. <i>This method is intended
    ...
    /**
     * Reads a signed short value out {@link #b b}. <i>This method is intended
     * for {@link Attribute} sub classes, and is normally not needed by class
     * generators or adapters.</i>
     *
     * @param index
    ...
    /**
     * Reads a signed int value out {@link #b b}. <i>This method is intended for
     * {@link Attribute} sub classes, and is normally not needed by class
     * generators or adapters.</i>
     *
     * @param index
    ...
    /**
     * Reads a signed long value out {@link #b b}. <i>This method is intended for
     * {@link Attribute} sub classes, and is normally not needed by class
     * generators or adapters.</i>
     *
     * @param index
    ...
    /**
     * Reads an UTF8 string constant pool item out {@link #b b}. <i>This method
     * is intended for {@link Attribute} sub classes, and is normally not needed
     * by class generators or adapters.</i>
     *
     * @param index
     *            the start index of an unsigned short value out {@link #b b},
    ...
    /**
     * Reads UTF8 string out {@link #b b}.
     *
     * @param index
    ...
    /**
     * Reads a class constant pool item out {@link #b b}. <i>This method is
    ...
    public String readClass(int index, char[] buf) {
        // computes the start index of the CONSTANT_Class item out b
    ...
    /**
     * Reads a numeric or string constant pool item out {@link #b b}. <i>This
